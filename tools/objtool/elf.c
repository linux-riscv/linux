					...
					unsigned int nr_relocs)
{
	...
	sec->rsec = rsec;
	rsec->base = sec;
	/* Relocations objtool creates are looked up through the hash. */
	rsec->hashed = true;
			       ...
			       unsigned int type)
{
	...
	/* A section read from the file is indexed, once grown it is hashed. */
	copy_reloc_cache_to_hash(elf, rsec, sec_num_entries(rsec));
