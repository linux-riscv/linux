...
static irqreturn_t riscv_iommu_pmu_irq_handler(int irq, void *dev_id)
{
	 ...
	 * Deliberately leave the hardware alone: the iommu only raises a new
	 * interrupt on a 0->1 transition of both the counter's overflow bit and
	 * ipsr.pmip, so clearing either one here would lose the overflow, and
	 * leaving them set means no further interrupt can arrive while the work
	 * is pending.
	 */
