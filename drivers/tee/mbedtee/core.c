...
static void mbedtee_release(struct tee_context *ctx)
{
	...
	if (d->is_supp_ctx) {
		...
		list_for_each_entry_safe(s, n, &d->sess_list, list_node) {
			u32 session_id = s->session_id;

			if (!mbedtee_close_session(ctx, session_id))
				continue;

			/*
			 * A non-zero return can mean the session was already
			 * dropped (e.g. -ESHUTDOWN during teardown) or kept on a
			 * transient error. Re-look it up: kfree(NULL) is a no-op
			 * in the former case, a force-removal in the latter.
			 */
			mutex_lock(&d->mutex);
			s = mbedtee_find_session_locked(d, session_id);
			if (s)
...
static int mbedtee_probe(struct platform_device *pdev)
{
	...
	xa_init_flags(&mbedtee->rpc_calls, XA_FLAGS_ALLOC1 | XA_FLAGS_LOCK_IRQ);
	mutex_init(&mbedtee->rpc_calls_mutex);
...
err_supp:
	...
	mbedtee_rpc_calls_release(mbedtee);
	return ret;
}
...
static void mbedtee_remove(struct platform_device *pdev)
{
	...
	/*
	 * Quiesce and wake all RPC waiters before tee_device_unregister(),
	 * which blocks until every context is released; a waiter still parked
	 * in a completion would otherwise deadlock teardown.
	 */
