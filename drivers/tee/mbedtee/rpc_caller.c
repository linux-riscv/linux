...
#define MBEDTEE_RPC_CALL_ABORTED		3

static void mbedtee_rpc_call_put(struct mbedtee_rpc_call *call)
{
	if (refcount_dec_and_test(&call->refs))
		kfree(call);
}
			   ...
			   size_t payload_size, struct mbedtee_rpc_call **call)
{
	...
	/* One reference for the caller, one for the XArray entry. */
	refcount_set(&rcall->refs, 2);
			   ...
			   struct mbedtee_rpc_call *call)
{
	...
	/*
	 * Interrupted calls remain reachable by the TEE, while aborted calls
	 * retain a quarantined remote reference. In both cases only release the
	 * caller reference here.
	 */
	xa_lock_irqsave(&mbedtee->rpc_calls, flags);
	if (call->state != MBEDTEE_RPC_CALL_INTERRUPTED &&
	    call->state != MBEDTEE_RPC_CALL_ABORTED &&
	    xa_load(&mbedtee->rpc_calls, rpc_index) == call) {
...
/*
 * Abort all outstanding calls at teardown: erase each entry and wake its
 * waiter so that no workqueue item can remain blocked in a completion
 * wait when destroy_workqueue() runs.
 */
void mbedtee_rpc_calls_abort(struct mbedtee_device *mbedtee)
{
	struct mbedtee_rpc_call *call;
	unsigned long flags;
	unsigned long idx;
	...
	mutex_lock(&mbedtee->rpc_calls_mutex);
	WRITE_ONCE(mbedtee->stopping, true);
	/*
	 * Hold xa_lock across the whole walk so a concurrent
	 * mbedtee_rpc_complete_call() cannot erase entries mid-iteration.
	 * xa_for_each() only takes RCU internally, so it does not recurse
	 * on xa_lock.
	 */
	xa_lock_irqsave(&mbedtee->rpc_calls, flags);
	xa_for_each(&mbedtee->rpc_calls, idx, call) {
		if (xchg(&call->state, MBEDTEE_RPC_CALL_ABORTED) ==
		    MBEDTEE_RPC_CALL_ACTIVE)
			complete(&call->c);
	}
	xa_unlock_irqrestore(&mbedtee->rpc_calls, flags);
	mutex_unlock(&mbedtee->rpc_calls_mutex);
...
/*
 * Release the XArray at teardown. Any entries left here are calls the TEE
 * never completed; it may still hold their rpc_phys, so we deliberately
 * leak (quarantine) them rather than risk a cross-world use-after-free.
 */
void mbedtee_rpc_calls_release(struct mbedtee_device *mbedtee)
{
	struct mbedtee_rpc_call *call;
	unsigned long idx;
	unsigned int quarantined = 0;

	xa_for_each(&mbedtee->rpc_calls, idx, call) {
		xa_erase(&mbedtee->rpc_calls, idx);
		/* No quiesce ACK exists, so retain the remote reference. */
		quarantined++;
	}
	xa_destroy(&mbedtee->rpc_calls);
	mutex_destroy(&mbedtee->rpc_calls_mutex);

	if (quarantined)
		dev_warn(mbedtee->dev, "quarantined %u RPC calls at release\n",
			 quarantined);
}
			       ...
			       u64 waiter_id)
{
	...
	/*
	 * Wake the waiter before dropping the XArray reference: a killed
	 * waiter that found the entry erased may release its own reference
	 * concurrently, so the put must be the last touch of the call.
	 */
				     ...
				     struct mbedtee_rpc_call *call, bool killable)
{
	...
	if (!killable) {
		...
		return READ_ONCE(call->state) == MBEDTEE_RPC_CALL_ABORTED ?
			-ESHUTDOWN : 0;
	}
	...
	if (xa_load(&mbedtee->rpc_calls, rpc_index) != call) {
		...
		xa_unlock_irqrestore(&mbedtee->rpc_calls, flags);
		return state == MBEDTEE_RPC_CALL_ABORTED ? -ESHUTDOWN : 0;
	}
	...
	xa_unlock_irqrestore(&mbedtee->rpc_calls, flags);
	return state == MBEDTEE_RPC_CALL_ABORTED ? -ESHUTDOWN : ret;
}

/*
 * Non-interruptible wait bounded by @timeout: on expiry the call is marked
 * INTERRUPTED (remote ref retained) and -ETIMEDOUT is returned, so a hung
 * TEE cannot block the caller forever.
 */
long mbedtee_rpc_wait_for_completion_timeout(struct mbedtee_device *mbedtee,
					     struct mbedtee_rpc_call *call,
					     unsigned long timeout)
{
	...
	if (wait_for_completion_timeout(&call->c, timeout))
		return READ_ONCE(call->state) == MBEDTEE_RPC_CALL_ABORTED ?
			-ESHUTDOWN : 0;
	...
	state = cmpxchg(&call->state, MBEDTEE_RPC_CALL_ACTIVE,
			MBEDTEE_RPC_CALL_INTERRUPTED);
	if (state == MBEDTEE_RPC_CALL_ACTIVE)
		/* Publish interruption before the TEE observes the request. */
		smp_store_release(&call->rpc.interrupted, true);
	xa_unlock_irqrestore(&mbedtee->rpc_calls, flags);
				...
				size_t num_params, const struct rpc_param *rp)
{
	...
	for (n = 0; n < num_params; n++) {
		...
		/*
		 * The TEE must not change the parameter type: the generic
		 * TEE layer releases memrefs based on the attributes the
		 * client supplied, so a type change here would leak the
			...
			struct tee_param *param)
{
	...
	if (ret != 0) {
		/*
		 * On -ERESTARTSYS/-ETIMEDOUT/-ESHUTDOWN the call is INTERRUPTED
		 * or ABORTED, so free() drops only the caller reference; the
		 * XArray keeps the remote reference until the TEE completes it.
		 */
		arg->ret = TEEC_ERROR_COMMUNICATION;
		arg->ret_origin = TEEC_ORIGIN_COMMS;
		mbedtee_rpc_call_free(mbedtee, call);
	...
	if (mbedtee_param_decode(param, arg->num_params, rp)) {
		arg->ret = TEEC_ERROR_COMMUNICATION;
		arg->ret_origin = TEEC_ORIGIN_COMMS;
	} else {
		arg->ret = (u32)call->rpc.ret;
...
out_session:
	remove = ret == 0 || READ_ONCE(mbedtee->stopping);
	mutex_lock(&ctxdata->mutex);
	sess = mbedtee_find_session_locked(ctxdata, session);
	if (sess) {
		if (remove)
