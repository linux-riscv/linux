...
#ifdef CONFIG_RISCV_ISA_SVNAPOT
/*
 * When called from set_huge_pte_at(), *ptep and pte do not encode the mapping
 * size since it will be set by __set_ptes(), so we need to rely on size.
 * When called from huge_ptep_get(), *ptep will contain the mapping size so
